...
class AWException extends Error {
  constructor(message: string, error?: Error) {
    console.warn(`[activity-watch] ${message}`);
    super(message);
  }
}
...
export default class ActivityWatchPlugin extends Plugin {
  ...
  bucket_id: string;
  sleep_time_seconds = 5; // loop cycle time
  ...
  async init() {
    ...
    if (this.settings.devServer) {
      console.log(
        `sleep_time_seconds is ${this.sleep_time_seconds}` +
          (this.sleep_time_seconds <= 0 ? ', skipping any timed heartbeats' : '')
      );
  ...
  async post(endpoint: string, data: object) {
    ...
    try {
      ...
      throw new AWException(
        `Request to URL [${r.url}] using [${r.method}], Header [${r.headers}], Body [${r.body}] failed!`,
        e
      );
    }
  }
  ...
  async sendData(id: string, heartbeat_data: object, pulse_time: number) {
    const endpoint = `buckets/${id}/heartbeat?pulsetime=${pulse_time}`;
    ...
    if (file) {
      await this.sendData(
        this.bucket_id,
        {
          ...
          projectPath:
            file.vault.adapter instanceof FileSystemAdapter ? file.vault.adapter.getBasePath() : 'unknown vault path',
          editor: 'Obsidian',
          editorVersion: apiVersion,
          ...(extraData ? extraData : {})
        },
        pulseTime
      ); // Yes this is sent exactly like a heartbeat. Heartbeats can be 0 duration and will split the timeline at that point in time. According to ActivityWatch developers, this is a valid way of doing things.
    }
  }

  async sendFileHeartbeatEvent(file: Nullable<TAbstractFile>) {
    await this.sendAbstractFileEvent(
      file,
      {
        eventType: 'obsidian.activeFileHeartbeatEvent'
      },
      this.sleep_time_seconds + 1
    );
  }
...
class ObsidianWatcherSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .addToggle((t) =>
        t.setValue(this.plugin.settings.devServer).onChange(async (value) => {
